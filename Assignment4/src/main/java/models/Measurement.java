...
public class Measurement {
    ...
    public static Measurement fromLine(String textLine, Map<Integer,Station> stations) {
        ...
        for (int i = 2; i < values.length ; i++) {
            try{
                values[i] = Double.parseDouble(fields[i].trim()) / 10;
        ...
        measurement.setAverageWindSpeed(values[FIELD_FG]);
        measurement.setMaxWindGust(values[FIELD_FXX]);
        measurement.setAverageTemperature(values[FIELD_TG]);
        measurement.setMinTemperature(values[FIELD_TN]);
        measurement.setMaxTemperature(values[FIELD_TX]);
        measurement.setSolarHours(values[FIELD_SQ] == -1 ? 0.0 : values[FIELD_SQ]);
        measurement.setPrecipitation(values[FIELD_RH] == -1 ? 0.0 : values[FIELD_RH]);
        measurement.setMaxHourlyPrecipitation(values[FIELD_RHX] == -1 ? 0.0 : values[FIELD_RHX]);
